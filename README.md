...
- ✅ **Smart Defaults in Checklist** - Already installed items start unchecked
...
- **Turso CLI** - Database CLI - *Optional*
- **Render CLI** - Deployments CLI - *Optional*
- **Sentry CLI** - Error tracking CLI - *Optional*
...
5. Ask whether to run service logins (Turso/Render/Sentry) at the end when running in an interactive terminal
...
│ Already installed items start unchecked        │
│                                                 │
│ === SYSTEM BASE ===                            │
│ [X] git                                        │
│ [X] build-essential                            │
...
### Service login prompts after installation

At the end of installation, the script can run login flows for:
- `turso auth login`
- `render login`
- `sentry-cli login`

If Part 2 runs automatically via `systemd` after reboot (no interactive TTY), login prompts are skipped and you can run those commands manually later.
