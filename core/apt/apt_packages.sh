...
install_apt_packages() {
    ...
    if [ "$INSTALL_VIVALDI" = true ]; then
        if command -v vivaldi-stable &>/dev/null || dpkg -s vivaldi-stable &>/dev/null; then
            print_success "Vivaldi (APT) already installed"
        else
            print_step "Installing Vivaldi (APT .deb)"
            if ! command -v gpg &>/dev/null; then
                apt install -y gnupg
            fi
            mkdir -p /etc/apt/keyrings
            curl -fsSL https://repo.vivaldi.com/archive/linux_signing_key.pub | gpg --dearmor -o /etc/apt/keyrings/vivaldi.gpg
            echo "deb [signed-by=/etc/apt/keyrings/vivaldi.gpg] https://repo.vivaldi.com/archive/deb/ stable main" > /etc/apt/sources.list.d/vivaldi.list
            apt update -y
