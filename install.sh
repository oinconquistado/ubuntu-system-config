    ...
    while IFS= read -r line; do
        ...
        case "$key" in
            CONFIG_USERNAME|CONFIG_HOME|PART1_COMPLETION_DATE)
                ...
                ;;
            *)
                if is_state_bool_key "$key" && [[ "$value" == "true" || "$value" == "false" ]]; then
...
save_state_file() {
    {
        ...
        printf 'CONFIG_HOME=%q\n' "${CONFIG_HOME}"

        local bool_key
        for bool_key in \
            INSTALL_GIT INSTALL_BUILD_ESSENTIAL INSTALL_GEDIT INSTALL_FUSE \
            INSTALL_NVM_NODE INSTALL_PNPM INSTALL_DENO INSTALL_BUN INSTALL_HOMEBREW INSTALL_GH_CLI \
            INSTALL_ZSH INSTALL_OH_MY_ZSH INSTALL_ZSH_PLUGINS \
            INSTALL_VSCODIUM INSTALL_VSCODE INSTALL_FLATPAK INSTALL_WEZTERM INSTALL_VIVALDI INSTALL_BITWARDEN INSTALL_STIMULATOR \
            FLATPAK_JUST_INSTALLED PART1_COMPLETED PART2_COMPLETED; do
            local bool_value="${!bool_key:-false}"
            [[ "$bool_value" == "true" || "$bool_value" == "false" ]] || bool_value=false
            printf '%s=%s\n' "$bool_key" "$bool_value"
        done

        printf 'PART1_COMPLETION_DATE=%q\n' "${PART1_COMPLETION_DATE}"
    } > "$STATE_FILE"
}
